  ...
  buyGoods(e) {
    ...
    if (latestFile) {
      const loadHandler = async (e) => {
        ...
        const objUser = await this.userDB.queryUser(localStorage.getItem('userID'));
        ...
        for (const item of sheetData) {
          ...
          if (item[dataHeader.idsp]) {
            ...
            // Khi bạn cập nhật số lượng cho từng kích cỡ:
            productSize[size.size] = (productSize[size.size] || 0) + size.quantity;
            const dataProduct = {
